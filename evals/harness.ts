...
import type { EvalCase, EvalOutcome, ExecutedQuery, GradeResult } from "./types";
...
export async function runCase(kase: EvalCase, client: ModelClient): Promise<EvalOutcome> {
  ...
  return { events, requests: wrapped.requests, finalText, executedSql };
}
...
export async function runCaseNTimes(
  ...
  client: ModelClient,
  n: number
): Promise<{ passRate: number; results: GradeResult[] }> {
  const results: GradeResult[] = [];
  for (let i = 0; i < n; i++) {
    const outcome = await runCase(kase, client);
