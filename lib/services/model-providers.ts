 ...
 *
 * Order: the company's configured provider (any user in the company shares
 * it — there is no per-user key), then the environment. The environment
 * fallback is what keeps a deployment that only ever set `ANTHROPIC_API_KEY`
 * working untouched — configuring a provider in the UI is an upgrade, not a
 * migration.
 *
